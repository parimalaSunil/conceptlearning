...
const Studentreview: React.FC = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        setLoading(true); // Start loading
        const response = await fetch('/data/studentReviews.json'); // Adjust path to where your JSON file is located
        const data = await response.json();
        
        // Assuming the structure is as expected
        setReviews(data.studentReviews || []);
      } catch (err) {
        setError('Something went wrong.');
      }
      setLoading(false); // Stop loading once data is fetched
      ...
      <div className="container mx-auto flex justify-center items-center">
        ...
        >
          <div className="flex items-center gap-2">
            <span>Parent <span className='text-blue-600 group-hover:text-white'>Testimonials</span></span>
            <IoMdArrowRoundForward className="group-hover:text-white" />
          </div>
        </Link>
      </div>
